    ...
    <section id="contacto" className="section-padding bg-cream">
      <div className="container-narrow">
        ...
        <div className="grid lg:grid-cols-5 gap-10 mt-16">
          <form onSubmit={onSubmit} className="reveal lg:col-span-3 bg-white p-8 md:p-10 rounded-2xl shadow-[var(--shadow-card)] space-y-5">
          ...
          <aside className="reveal lg:col-span-2 space-y-4">
            <div className="bg-navy text-cream p-8 md:p-10 rounded-2xl">
              <h3 className="font-display text-2xl text-gold mb-6">Información de contacto</h3>
              <ul className="space-y-5">
                {contactInfo.map((c) => (
                  <li key={c.label} className="flex items-start gap-4">
                    ...
                    {c.href ? (
                      <a href={c.href} className="text-cream/90 hover:text-gold transition-colors leading-relaxed">{c.label}</a>
                    ) : (
                      <span className="text-cream/90 leading-relaxed">{c.label}</span>
                    )}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-gold/15 border border-gold/40 p-6 rounded-2xl">
              <p className="text-navy text-sm leading-relaxed">
                <strong className="font-display text-lg block mb-1 text-navy">Empresa familiar</strong>
                Más de 20 años cuidando los muebles de las familias del Vallès. Atención cercana, presupuestos honestos y trabajo bien hecho.
              </p>
            </div>
          </aside>
        </div>
